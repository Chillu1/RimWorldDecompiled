...
public class QuestNode_Root_Gravcore_FrozenTerraformer : QuestNode_Root_Gravcore
{
	protected override void RunInt()
	{
		...
		{
			Log.Error("Could not find valid site tile for frozen terraformer quest.");
			return;
		}
		...
		{
			new SitePartDefWithParams(SitePartDefOf.FrozenTerraformer, new SitePartParams
			{
		...
		quest.Letter(LetterDefOf.NeutralEvent, text, null, null, null, useColonistsFromCaravanArg: false, QuestPart.SignalListenMode.OngoingOnly, label: "FrozenTerraformerArrivedLetter".Translate(), text: "FrozenTerraformerArrivedLetterText".Translate(), lookTargets: Gen.YieldSingle(site.Map));
